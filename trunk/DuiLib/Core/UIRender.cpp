...
namespace DuiLib {
	...
	HRGN	CRenderEngine::CreateImageRegion(TImageInfo* pImageInfo, COLORREF cTranColor)
	{
		//判断状态
		...
		{
			void * pbits32=NULL; 
			HBITMAP hBitmap=pImageInfo->hBitmap;
			BITMAPINFOHEADER RGB32BITSBITMAPINFO={sizeof(BITMAPINFOHEADER),pImageInfo->nX,pImageInfo->nY,1,32,BI_RGB,0,0,0,0,0};
			...
			if (hbm32)
			{
				...
				if (hDC)
				{
					...
					::BitBlt(hMemDC,0,0,pImageInfo->nX,pImageInfo->nY,hDC,0,0,SRCCOPY);
					DWORD maxRects=100;
					...
					for (int y=0;y<pImageInfo->nY;y++)
					{
						for (int x=0;x<pImageInfo->nX;x++)
						{
							int x0=x;
							LONG *p=(LONG *)p32+x;
							while (x<pImageInfo->nX)
							{
								BYTE b=GetRValue(*p);
								if ((b>=hr)&&(b<=hr))
								{
